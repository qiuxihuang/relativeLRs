...
source("R program/Sample size calculation/1_n_rlr_fxn.R")
source("R program/Sample size calculation/2_n_logrlr_fxn.R")
  ...
  for (OR in true_or){
    for (tau2 in Tau2){
      for (rho in Rho){
        ...
        rslts <- read.table(paste0("intermediate results/pval_n_", n, "_k_", k, "_OR_", OR, "_tau2_", tau2, "_rho_", rho0, 
                                   ".txt"), header=T, sep="")
        # set nominal = 0.1
